---
name: icp-definition
description: "Use when stage 2 of the prospecting system needs the ICP written down — hard filters, soft preferences, offer, and exclusions — starting from the customer patterns found in stage 1. Also use standalone when a website or a plain description is the only input."
license: MIT
metadata:
  author: automatewithuday
  source: martechs.io
  ported_from: GTM-Skills/skills/icp-onboarding/SKILL.md
---
...
**Stage 2 of 20.** Reads `01-customer-patterns.md` and `00-customers.csv`. Writes `02-client-profile.yaml`.

- Pre-fill the interview from `01-customer-patterns.md` — propose answers, let the human correct them. Never skip the hard-vs-soft split.
- Put every domain from `00-customers.csv` into `excluded_domains`.
- Save to the run folder (`runs/<run>/02-client-profile.yaml`), not the `~/cold-email-ai-skills/` path below.
- Tiering and persona depth: `references/icp-matrix-builder.md`, `references/persona-development.md`.
- Done when the human has confirmed the yaml.
...
10. **Lead magnet — what can you legally give away for free?** (If they don't have one, invoke [`/lead-magnet-brainstorm`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/lead-magnet-brainstorm) skill.)
...
With `client-profile.yaml` in hand, the user's next skill is [`/cold-email-starter-kit`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/cold-email-starter-kit) (manual first campaign) or [`/auto-research-public`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/auto-research-public) (automated daily campaigns).

## Files

- `references/prospeo-industries.md` — full list of 256 valid Prospeo industry names
- `references/example-profiles/` — 3 example client-profile.yaml files (B2B SaaS, local SMB, agency) *(not shipped yet)*
