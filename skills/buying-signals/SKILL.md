---
name: buying-signals
description: "Use when stage 9 of the prospecting system needs buying signals attached to qualified accounts — funding, hiring, new-in-role, tech changes — each with a source URL and a real event date. Also use standalone to design a signal taxonomy and scoring model."
license: MIT
metadata:
  author: automatewithuday
  source: martechs.io
  ported_from: gtm-flywheel/signal-scoring/intent-signals/SKILL.md
---
...
**Stage 9 of 20.** Reads `08-leads.csv`. Writes `09-signals.csv` (`account_domain, signal_type, signal_detail, source_url, event_date, collected_date, confidence`).
