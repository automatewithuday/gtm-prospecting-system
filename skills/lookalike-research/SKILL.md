---
name: lookalike-research
description: "Use when stage 3 of the prospecting system needs to turn seed customer domains into lookalike companies. Also use standalone when a handful of known-good domains or a plain-language ICP should become hundreds of similar companies via DiscoLike."
license: MIT
metadata:
  author: automatewithuday
  source: martechs.io
  ported_from: GTM-Skills/skills/disco-like/SKILL.md
---
...
**Stage 3 of 20.** Reads `01-seeds.csv` and `02-client-profile.yaml`. Writes `03-lookalikes.csv` (columns below, `source=discolike`).

- Pass `excluded_domains` from the profile as `--negation-domains`.
- Sample 50–100 first and qualify with [`/audience-filter`](../audience-filter/SKILL.md) before scaling (the required step below).
- No `DISCOLIKE_API_KEY`? Skip this stage: [`/targeted-search`](../targeted-search/SKILL.md) runs its own lookalike pass (Exa / Parallel) from the same seeds.
- `03-lookalikes.csv` enters stage 7 through `extra_candidates` in `lane.json`.
- Turning one new win into lookalikes on a trigger: `references/closed-won-replication-play.md`.
...
Before pulling 5,000 companies, run DiscoLike on a small sample (50-100), then invoke [`/audience-filter`](../audience-filter/SKILL.md):
...
1. [`/icp-definition`](../icp-definition/SKILL.md) → nail down seed companies (your best 5 customers)
2. `/disco-like --domains="seed1,seed2,..." --limit=100 --out=sample.csv` → sample run
3. [`/audience-filter`](../audience-filter/SKILL.md) → score the sample, tune ICP prompt
...
- `scripts/count.ts` — pre-check universe size before paying *(not shipped yet; use `discover.ts` with a small `--limit` to probe)*
- `scripts/bizdata.ts` — single-domain lookup *(not shipped yet)*

## What to do next

**Run [`/audience-filter`](../audience-filter/SKILL.md)** on your 50-company sample (required step above). Then either:
- [`/blitz-list-builder`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/blitz-list-builder) to find owner contacts at each filtered domain, OR
- [`/list-quality-scorecard`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/list-quality-scorecard) directly if this is companies-only and you'll enrich another way

**Or wait:** if the 50-sample ICP fit was poor (<40% matches), don't scale. Change your seed domains and re-run with better inputs.

## Related skills

- [`/icp-definition`](../icp-definition/SKILL.md) — defines the seed domains you'll use
- [`/audience-filter`](../audience-filter/SKILL.md) — quality-check the first 50 results before scaling
- [`/blitz-list-builder`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/blitz-list-builder) — chain to find contacts at each discovered company
- `/email-waterfall` — fill missing emails after Blitz
- [`/cold-email-starter-kit`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/cold-email-starter-kit) → `06-list-building-prospeo.md` for broader list-building patterns
