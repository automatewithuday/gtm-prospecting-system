...
description: "Use when stage 17 of the prospecting system needs campaign replies classified (interested, soft positive, referral, neutral, negative, OOO, bounce, unsubscribe) and the positive reply rate computed against the 1% bar. Also use when raw reply rate looks fine but pipeline is not showing up."
license: MIT
metadata:
  author: automatewithuday
  source: martechs.io
  ported_from: GTM-Skills/skills/positive-reply-scoring/SKILL.md
---
...
**Stage 17 of 20.** Runs after the human has sent the campaign. Reads replies from Smartlead (scripts below) OR `17-replies-raw.csv` (`lead_id, email, reply_text, replied_at`) exported from any sender. Writes `17-replies.csv` (adds `label`, `is_positive`, `next_action`) and `17-reply-score.json`.

- Save to the run folder, not the `~/cold-email-ai-skills/` path below.
- `unsubscribe` and `negative_hostile` rows go to the do-not-contact list in the same session — that list feeds stage 10's hard disqualifiers.
- Positive rows hand off to [`/lead-qualification`](../lead-qualification/SKILL.md).
...
**Respond to every `positive_interested` reply within 30 seconds** of seeing it. Then [`/experiment-design`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/experiment-design) to plan the next iteration based on what worked.

**If positive reply rate is <1% after 200+ sends:** the 1% rule failed. Run [`/email-deliverability-audit`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/email-deliverability-audit) (are you reaching the inbox?) (check for vague CTAs, generic first lines, em dashes).

**Or wait:** this skill is the Wednesday task in [`/pipeline-review`](../pipeline-review/SKILL.md). Run it weekly going forward.

## Related skills

- [`/experiment-design`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/experiment-design) — uses positive reply rate as the success metric
- [`/email-deliverability-audit`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/email-deliverability-audit) — if hostile + unsub are elevated, run this next
- [`/cold-email-starter-kit`](https://github.com/automatewithuday/GTM-Skills/tree/main/skills/cold-email-starter-kit) → `10-reply-handling.md` for what to do with the positive replies once flagged
