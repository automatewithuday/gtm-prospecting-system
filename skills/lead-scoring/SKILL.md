---
name: lead-scoring
description: "Use when stage 10 of the prospecting system needs every lead screened against hard disqualifiers and given a fit score with its components shown. Also use standalone to decide whether a target account is worth pursuing."
license: MIT
metadata:
  author: automatewithuday
  source: martechs.io
  ported_from: gtm-flywheel/icp-research/account-qualification/SKILL.md
---
...
**Stage 10 of 20.** Reads `08-leads.csv`, `02-client-profile.yaml`. Writes `10-scored.csv` (adds `screen` pass/fail/review, `screen_reason`, `fit_score` 0-100, `fit_components`).

- Run Level 1 on every row, Level 2 on rows that pass. Level 3 belongs to [`/lead-qualification`](../lead-qualification/SKILL.md) and [`/account-notes`](../account-notes/SKILL.md).
- This stage scores FIT only. Intent is stage 9's data and is combined in stage 11 — keep them in separate columns.
- Missing data stays blank and scores as unknown, never as zero. Put what was missing in `fit_components`.
- Weights come from the profile's hard/soft split. Propose weights to the human once per run; do not invent new ones mid-run.
