---
name: follow-ups
description: "Use when stage 16 of the prospecting system needs the follow-up steps, timing, and channel mix drafted around the first-touch email and LinkedIn note. Also use standalone to design a multi-step outbound sequence."
license: MIT
metadata:
  author: automatewithuday
  source: martechs.io
  ported_from: gtm-flywheel/cold-email/sequence-architecture/SKILL.md
---
...
**Stage 16 of 20.** Reads `14-emails.csv`, `15-linkedin.csv`. Writes `16-sequence.md` (steps, day offsets, channel per step) and `16-followups.csv` (`segment, step, day_offset, channel, subject, body`).

- Each follow-up adds something new (proof, angle, resource). A bump that only says "following up" does not ship.
- Follow-up copy rules live in [`/cold-email`](../cold-email/SKILL.md) section 7; this skill owns order, timing, and escalation.
- Drafts only. Sequence settings are a recommendation for the human's sender.
