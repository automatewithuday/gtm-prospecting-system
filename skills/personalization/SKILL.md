...
description: "Use when stage 13 of the prospecting system needs the strongest personalization angle per lead turned into short custom variables, generated in parallel with a human approval loop. Also use standalone to design a personalization hierarchy and variable set."
license: MIT
metadata:
  author: automatewithuday
  source: martechs.io
  ported_from: gtm-flywheel/cold-email/personalization-engine/SKILL.md
---
...
**Stage 13 of 20.** Reads `12-segmented.csv`, `09-signals.csv`. Writes `13-personalized.csv` (adds `angle_source`, `situation_line`, `value_line`, `cta_soft`).
