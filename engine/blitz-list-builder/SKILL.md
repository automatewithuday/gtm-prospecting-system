...
description: "Use when a list of company domains already exists (competitors, Google Maps export, named accounts) and the job is finding the owner, founder, CEO, or VP at each one via the Blitz API. Best for SMB owner-finding and named-account ABM. Also use when Prospeo's title-first search misses small companies."
license: MIT
...
Before pulling more than 500 companies' worth of contacts, sample ~50 and run [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) on them:
...
**How to skip safely:** you can't. If you're sure your list is already perfectly qualified (e.g., it came from a curated seed list of named accounts you want to pitch), run [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) anyway on 10 of them and confirm your existing qualification matches — the prompt builder becomes a sanity check instead of a filter.
...
5. **Upload** — pass to Smartlead via [`/cold-email-starter-kit`](../cold-email-starter-kit/SKILL.md) script `smartlead-add-leads.ts`
...
**Run `/email-waterfall`** to fill in emails for rows Blitz didn't return. Then [`/list-quality-scorecard`](../list-quality-scorecard/SKILL.md) to grade the final list.

After the scorecard, proceed to [`/campaign-copywriting`](../campaign-copywriting/SKILL.md) → [`/smartlead-campaign-upload-public`](../smartlead-campaign-upload-public/SKILL.md).

**Or wait:** if the required [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) step (above) showed <40% qualification rate, your source list of domains is off. Fix that before spending more Blitz credits.

## Related skills

- [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) — qualify the contacts this skill returns BEFORE running email waterfall
- [`/prospeo-full-export`](../prospeo-full-export/SKILL.md) — the title-first alternative
- [`/google-maps-list-builder`](../google-maps-list-builder/SKILL.md) — for local-business domain lists
- [`/disco-like`](../disco-like/SKILL.md) — lookalike companies from seed domains
- `/email-waterfall` — when Blitz doesn't return an email
- [`/cold-email-starter-kit`](../cold-email-starter-kit/SKILL.md) → `06-list-building-prospeo.md` for broader list-building context
