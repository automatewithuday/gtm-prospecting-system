---
name: list-expander
description: "Use when a niche list feels incomplete, known good-fit companies are missing from keyword searches, or ~10 seed companies need to become a full qualified TAM. Also use when the question is how good-fit companies are actually tagged in the database so filters can be widened."
license: MIT
...
Tune the qualification prompt FIRST via [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) (interactive, 10-company batches,
...
## Related skills
- [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) — tune the qualification prompt before Phase 4 (the score-batch script it
  refers to lives HERE: `scripts/score-batch.ts`)
- [`/prospeo-search-api`](../prospeo-search-api/SKILL.md) — full Prospeo filter reference
- [`/prospeo-full-export`](../prospeo-full-export/SKILL.md) — title-first paginated lead export once you have the company list
- [`/blitz-list-builder`](../blitz-list-builder/SKILL.md) — domain-first contact discovery on the qualified companies
- [`/disco-like`](../disco-like/SKILL.md) — a 4th lookalike source (seed domains or NL ICP text)
- [`/list-quality-scorecard`](../list-quality-scorecard/SKILL.md) — grade the final CSV before it goes anywhere near a campaign
