---
name: list-builder
description: "Use when the request is \"build me a list\", \"get me leads for X\", or \"find everyone who...\" and the goal is the largest qualified list possible across Prospeo, GetLeads, Blitz, and lookalike snowballing, with AI qualification and uncapped contact pull. Requires list-expander installed alongside."
license: MIT
...
| Layer | Delegate |
|---|---|
| Company discovery + qualification + snowball | [`/list-expander`](../list-expander/SKILL.md) (scripts in its `scripts/`) |
| Prospeo filters/syntax | [`/prospeo-search-api`](../prospeo-search-api/SKILL.md) |
| Blitz domain→people | [`/blitz-list-builder`](../blitz-list-builder/SKILL.md) (`scripts/find-contacts.ts`) |
| GetLeads counts/exports | `scripts/getleads-client.ts` (counts are free) — optional, skipped without a key |
| Email validation | provider emails from GetLeads/Blitz/Prospeo → `leads-final.csv`; validate with MillionVerifier ([`/cold-email-starter-kit`](../cold-email-starter-kit/SKILL.md)) before upload |
| Judge tuning | [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) |
| Local/SMB discovery | [`/google-maps-list-builder`](../google-maps-list-builder/SKILL.md) |
| List QA before delivery | [`/list-quality-scorecard`](../list-quality-scorecard/SKILL.md) |
...
| `BLITZ_API_KEY` | domain→people fallback via [`/blitz-list-builder`](../blitz-list-builder/SKILL.md) (see that skill for signup) | BLITZ stage skips, Prospeo covers those domains |
| `EXA_API_KEY`, `PARALLEL_AI_API_KEY` | lookalike / entity discovery inside [`/list-expander`](../list-expander/SKILL.md) Phase 2 (sign up at https://exa.ai, https://parallel.ai) | those sub-sources are skipped, seeds + filters still run |
...
you upload, run the whole file through MillionVerifier (see [`/cold-email-starter-kit`](../cold-email-starter-kit/SKILL.md)) and
...
- **Seed-driven core** (when seeds exist — almost always): [`/list-expander`](../list-expander/SKILL.md) Phases 1–3
...
- **Google Maps** when local/SMB — [`/google-maps-list-builder`](../google-maps-list-builder/SKILL.md) (live scrape by category ×
...
- **Parallel entity-search / Exa findSimilar / DiscoLike** — already inside list-expander
  Phase 2 (see [`/disco-like`](../disco-like/SKILL.md)). For local-service TAMs, ALSO run a Parallel per-metro fan-out
...
1. **Live Maps by QUERY** (not category) — see [`/google-maps-list-builder`](../google-maps-list-builder/SKILL.md); ~350 cities ×
...
- Tune the judge once via [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) (10-company rounds until 2 clean).
...
2. **Blitz** (optional key): [`/blitz-list-builder`](../blitz-list-builder/SKILL.md) `scripts/find-contacts.ts` for domains
